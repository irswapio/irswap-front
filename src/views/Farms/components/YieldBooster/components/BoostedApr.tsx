...
function BoostedApr(props: BoostedAprPropsType) {
  ...
  const msg =
    boosterState === YieldBoosterState.ACTIVE ? (
      ...
      <>
        <Text bold color="#bb935c" {...rest} fontSize={14} display="inline-block" mr="3px">
          {t('Up to')}
        </Text>
  ...
  return (
    <Box display="flex" border="1px solid #bb935c" borderRadius="3xl" ml={2} py={1} px={2}>
      <RocketIcon m="4px" color="#bb935c" />
      <Text bold color="#bb935c" {...rest} fontSize={16}>
        {msg}
      </Text>
    </Box>
  )
